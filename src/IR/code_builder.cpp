...
namespace SiiIR {
  ...
  public:
    ...
    SiiIRBinaryOperationPtr append_divide( ValuePtr left,
                                           ValuePtr right ) override;
    SiiIRBinaryOperationPtr append_add( ValuePtr left,
                                        ValuePtr right ) override;
    SiiIRBinaryOperationPtr append_sub( ValuePtr left,
                                        ValuePtr right ) override;
    SiiIRUnaryOperationPtr  append_neg( ValuePtr left ) override;
    SiiIRBinaryOperationPtr append_equal( ValuePtr left,
                                          ValuePtr right ) override;
    ...
    SiiIRFunctionDefinitionPtr
                   append_function( FunctionValuePtr func ) override;
    ...
    SiiIRStorePtr  append_store( ValuePtr source,
                                 ValuePtr dest_address ) override;
  ...
  SiiIRBinaryOperationPtr CodeBuilderImpl::append_multiply( ValuePtr left,
    ...
    SiiIRBinaryOperationPtr new_code
        = std::make_shared< SiiIRBinaryOperation >( SiiIRCodeKind::MUL,
                                                    std::move( left ),
                                                    std::move( right ),
                                                    left->type_ );
  ...
  SiiIRBinaryOperationPtr CodeBuilderImpl::append_divide( ValuePtr left,
                                                          ValuePtr right ) {
    SiiIRBinaryOperationPtr new_code
        = std::make_shared< SiiIRBinaryOperation >( SiiIRCodeKind::DIV,
                                                    std::move( left ),
                                                    std::move( right ),
                                                    left->type_ );
    append_new_code( new_code );
  ...
  SiiIRBinaryOperationPtr CodeBuilderImpl::append_add( ValuePtr left,
                                                       ValuePtr right ) {
    SiiIRBinaryOperationPtr new_code
        = std::make_shared< SiiIRBinaryOperation >( SiiIRCodeKind::ADD,
                                                    std::move( left ),
                                                    std::move( right ),
                                                    left->type_ );
    append_new_code( new_code );
  ...
  SiiIRBinaryOperationPtr CodeBuilderImpl::append_sub( ValuePtr left,
                                                       ValuePtr right ) {
    SiiIRBinaryOperationPtr new_code
        = std::make_shared< SiiIRBinaryOperation >( SiiIRCodeKind::SUB,
                                                    std::move( left ),
                                                    std::move( right ),
                                                    left->type_ );
    append_new_code( new_code );
  ...
  SiiIRStorePtr CodeBuilderImpl::append_store( ValuePtr source,
    ...
    SiiIRStorePtr new_code
        = std::make_shared< SiiIRStore >( source, dest_address );
    append_new_code( new_code );
    return new_code;
  }
  ...
  SiiIRReturnPtr CodeBuilderImpl::append_return( ValuePtr value ) {
    SiiIRReturnPtr new_code
        = std::make_shared< SiiIRReturn >( std::move( value ) );
    append_new_code( new_code );
    return new_code;
  }
  ...
  CodeBuilderImpl::append_condition_branch( ValuePtr condition,
    ...
    SiiIRConditionBranchPtr new_code
        = std::make_shared< SiiIRConditionBranch >( std::move( condition ),
                                                    std::move( true_label ),
  ...
  SiiIRGotoPtr CodeBuilderImpl::append_goto( LabelPtr label ) {
    SiiIRGotoPtr then_goto
        = std::make_shared< SiiIRGoto >( std::move( label ) );
  ...
  SiiIRAllocaPtr CodeBuilderImpl::append_alloca( uint32_t bytes,
                                                 TypePtr  type ) {
