...
std::string SiiIRBinaryOperation::to_string(IDAllocator &id_allocator) const {
  ...
  static std::map<SiiIRCodeKind, std::string> binary_operator_str = {
      {SiiIRCodeKind::MUL, " * "},       {SiiIRCodeKind::DIV, " / "},
      {SiiIRCodeKind::ADD, " + "},       {SiiIRCodeKind::SUB, " - "},
      {SiiIRCodeKind::EQUAL, " == "},    {SiiIRCodeKind::NOT_EQUAL, " != "},
      {SiiIRCodeKind::LESS_THAN, " < "}, {SiiIRCodeKind::LESS_EQUAL, " <= "},
