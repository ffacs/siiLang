...
namespace SiiIR {
  ...
  std::string
  SiiIRBinaryOperation::to_string( IDAllocator& id_allocator ) const {
    ...
    if ( operator_str_iter == binary_operator_str.end() ) {
      ...
      error_str << "Unknown SiiIRCodeKind: "
  ...
  std::string
  SiiIRUnaryOperation::to_string( IDAllocator& id_allocator ) const {
    auto prefix = SiiIRCode::to_string( id_allocator );
  ...
  std::string
  SiiIRConditionBranch::to_string( IDAllocator& id_allocator ) const {
    auto prefix = SiiIRCode::to_string( id_allocator );
    return prefix + "  if " + id_allocator.alloc( condition_->value_.get() )
           + " goto " + id_allocator.alloc( true_label_->value_.get() )
           + " else " + id_allocator.alloc( false_label_->value_.get() ) + ";";
  }
  ...
  std::string SiiIRAlloca::to_string( IDAllocator& id_allocator ) const {
    return SiiIRCode::to_string( id_allocator ) + "  "
           + id_allocator.alloc( this ) + " = alloca size "
           + std::to_string( size_ ) + ";";
  }

  std::string SiiIRLoad::to_string( IDAllocator& id_allocator ) const {
    return SiiIRCode::to_string( id_allocator ) + "  "
           + id_allocator.alloc( this ) + " = load "
