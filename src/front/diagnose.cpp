...
namespace front {
  ...
  public:
    ...
    void mismatch( DiagnoseLevel level,
                   LexInfo       lex_info,
                   std::string   expected,
                   std::string   found ) override {
      handle(
          level, lex_info, "Expected " + expected + ", but found " + found );
    }

    void unexpect( DiagnoseLevel level,
                   LexInfo       lex_info,
  ...
  DiagnoseHandlerPtr CreateDiagnoseHandler( std::string      file_name,
    ...
    return std::make_shared< DiagnoseHandlerImpl >( std::move( file_name ),
                                                    content );
  }
