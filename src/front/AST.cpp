...
namespace front {
  ...
  ASTNode::Normalize_function_declaration( ASTNodePtr node ) {
    ...
    auto& declaration_statement_list
        = function_node.declaration_statement_list_;
    ...
    for ( const auto& declaration_statement : declaration_statement_list ) {
      for ( const auto& declaration :
            declaration_statement->declaration_list_ ) {
    ...
    if ( is_parameter_typed.value_or( false ) ) {
      ...
      for ( auto& parameter_declaration : declaration_list ) {
        ...
        auto aim_iter
            = name_to_parameter.find( declarator_in_node->identifier_ );
    ...
    declarator->type_
        = Type::NormalizeParameterDeclaration( declarator->type_ );
    return std::make_shared< FunctionDeclarationNode >( std::move( declarator ),
                                                        std::move( body ) );
  }
