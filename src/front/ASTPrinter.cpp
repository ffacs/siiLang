  ...
  switch (node.kind_) {
    ...
    case ASTNodeKind::IDENTIFIER: {
      const LiteralNode& literal_node =
          static_cast<const LiteralNode&>(node);
...
void ASTPrintVisitor::visit(const BinaryOperationNode &node) {
  switch (node.kind_) {
    ...
    case ASTNodeKind::LESS_EQUAL:
    case ASTNodeKind::ASSIGN: {
      os_ << indent_ << "BinaryOperationNode: " << BINARY_OPERATION_TYPE_TO_STRING[node.kind_] << "\n";
      ++indent_;
      visit(*node.lhs_);
      visit(*node.rhs_);
      --indent_;
    ...
    default: {
      std::stringstream error_msg;
      error_msg << "Unknow type of BinaryOperationNode" << static_cast<uint32_t>(node.kind_);
      throw std::invalid_argument(error_msg.str());
    }
  }
}
...
void ASTPrintVisitor::visit(const UnaryOperationNode &node) {
  switch (node.kind_) {
    ...
    default: {
      std::stringstream error_msg;
      error_msg << "Unknow type of UnaryOperationNode" << static_cast<uint32_t>(node.kind_);
      throw std::invalid_argument(error_msg.str());
    }
  }
}
...
void ASTPrintVisitor::visit(const LiteralNode &node) {
  switch (node.kind_) {
    case ASTNodeKind::INTEGER: {
      os_ << indent_ << "LiteralNode: " << " int " << node.literal_ << "\n";
      return;
    }
    case ASTNodeKind::IDENTIFIER: {
      os_ << indent_ << "LiteralNode: " << " identifier " << node.literal_ << "\n";
      return;
    }
    default: {
      std::stringstream error_msg;
      error_msg << "Unknow type of LiteralNode" << static_cast<uint32_t>(node.kind_);
      throw std::invalid_argument(error_msg.str());
    }
  }
}
