...
namespace front {
  ...
  protected:
    ...
    ValuePtr generate_for_integer_node( const ASTNodePtr& node,
                                        CodeBuilderPtr&   code_builder );
    VariableSymbolPtr
             generate_for_identifier_node( const ASTNodePtr& node,
                                           CodeBuilderPtr&   code_builder );
  ...
  IRGeneratorImpl::generate_for_add_node( const ASTNodePtr& node,
    ...
    auto right_value
        = generate_for_rvalue_node( binary_operation_node->rhs_, code_builder );
    auto result = code_builder->append_add( std::move( left_value ),
                                            std::move( right_value ) );
    return result;
  }
  ...
  IRGeneratorImpl::generate_for_sub_node( const ASTNodePtr& node,
    ...
    auto right_value
        = generate_for_rvalue_node( binary_operation_node->rhs_, code_builder );
    auto result = code_builder->append_sub( std::move( left_value ),
                                            std::move( right_value ) );
    return result;
  }
  ...
  IRGeneratorImpl::generate_for_neg_node( const ASTNodePtr& node,
    ...
    auto child_value = generate_for_rvalue_node( unary_operation_node->operand_,
                                                 code_builder );
    auto result      = code_builder->append_neg( std::move( child_value ) );
    return result;
  }

  ValuePtr IRGeneratorImpl::generate_for_get_address_node(
      const ASTNodePtr& node,
      CodeBuilderPtr&   code_builder ) {
    const UnaryOperationNode* unary_operation_node
        = static_cast< const UnaryOperationNode* >( node.get() );
    auto child_value = generate_for_lvalue_node( unary_operation_node->operand_,
                                                 code_builder );
  ...
  IRGeneratorImpl::generate_for_equal_node( const ASTNodePtr& node,
    ...
    auto right_value
        = generate_for_rvalue_node( binary_operation_node->rhs_, code_builder );
    auto result = code_builder->append_equal( std::move( left_value ),
                                              std::move( right_value ) );
    return result;
  }
  ...
  ValuePtr IRGeneratorImpl::generate_for_less_equal_node(
      const ASTNodePtr& node,
      CodeBuilderPtr&   code_builder ) {
    const BinaryOperationNode* binary_operation_node
        = static_cast< const BinaryOperationNode* >( node.get() );
  ...
  IRGeneratorImpl::generate_for_integer_node( const ASTNodePtr& node,
    ...
    return Value::constant( literal_node->literal_,
                            SiiIR::Type::Integer( 32 ) );
  }

  VariableSymbolPtr IRGeneratorImpl::generate_for_identifier_node(
      const ASTNodePtr& node,
      CodeBuilderPtr&   code_builder ) {
    const LiteralNode* literal_node
        = static_cast< const LiteralNode* >( node.get() );
  ...
  IRGeneratorImpl::generate_for_assign_node( const ASTNodePtr& node,
    ...
    if ( binary_operation_node->lhs_->kind_ != ASTNodeKind::IDENTIFIER ) {
      throw std::invalid_argument(
          "Expect Identifier on the left of assignment" );
    }
    auto right_value
        = generate_for_rvalue_node( binary_operation_node->rhs_, code_builder );
    ...
    if ( left_value->kind_ != SymbolKind::VARIABLE ) {
      throw std::invalid_argument(
          "Expect Variable on the left of assignment" );
    }
  ...
  void
  IRGeneratorImpl::generate_for_if_else_node( const ASTNodePtr& node,
                                              CodeBuilderPtr&   code_builder ) {
    const IfElseNode* if_else_node
        = static_cast< const IfElseNode* >( node.get() );
    ...
    SiiIR::SiiIRConditionBranchPtr branh
  ...
  void
  IRGeneratorImpl::generate_for_for_loop_node( const ASTNodePtr& node,
                                               CodeBuilderPtr& code_builder ) {
  ...
  void
  IRGeneratorImpl::generate_for_do_while_node( const ASTNodePtr& node,
                                               CodeBuilderPtr& code_builder ) {
  ...
  void IRGeneratorImpl::generate_for_variable_declaration_node(
    ...
    ctx_manager_->append_variable(
        name,
        Symbol::NewVariableSymbol( std::move( type ), allocated_address ) );
  ...
  FunctionValuePtr IRGeneratorImpl::generate_for_function_declaration_node(
    ...
    const auto& function_type = static_cast< const FunctionType& >(
        *function_node.declarator_->type_ );
    ...
    if ( function_node.body_ ) {
      ...
      for ( auto& parameter : function_type.parameter_types_ ) {
        ...
        body_builder->append_store( parameter_value,
    ...
    FunctionValuePtr function_value
        = Value::Function( function_codes,
                           function_ctx,
                           function_name,
                           std::move( ir_function_type ) );
