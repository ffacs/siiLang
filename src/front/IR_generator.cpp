...
protected:
  ...
  AddressPtr generate_for_integer_node(const ASTNodePtr &node,
                                       CodeBuilderPtr &code_builder);
  AddressPtr generate_for_identifier_node(const ASTNodePtr &node,
                                          CodeBuilderPtr &code_builder);
...
AddressPtr IRGeneratorImpl::generate_for_node(const ASTNodePtr &node,
  ...
  default:
    ...
    error_msg << "Unknow type of AST Node"
...
IRGeneratorImpl::generate_for_mul_node(const ASTNodePtr &node,
  ...
  auto left_address =
      generate_for_node(binary_operation_node->lhs_, code_builder);
  auto right_address =
      generate_for_node(binary_operation_node->rhs_, code_builder);
...
IRGeneratorImpl::generate_for_neg_node(const ASTNodePtr &node,
  ...
  auto child_address =
      generate_for_node(unary_operation_node->operand_, code_builder);
  auto result = ctx_manager_->function_ctx()->allocate_temporary_address();
...
IRGeneratorImpl::generate_for_assign_node(const ASTNodePtr &node,
  ...
  return code_builder->append_assign(std::move(left_address),
                                     std::move(right_address));
}
...
AddressPtr IRGeneratorImpl::generate_for_compound_statement_node(
...
AddressPtr IRGeneratorImpl::generate_for_declaration_statement_node(
    const ASTNodePtr &node, CodeBuilderPtr &code_builder) {
...
AddressPtr IRGeneratorImpl::generate_for_variable_declaration_node(
    const ASTNodePtr &node, CodeBuilderPtr &code_builder) {
...
AddressPtr IRGeneratorImpl::generate_for_function_declaration_node(
    const ASTNodePtr &node, CodeBuilderPtr &code_builder) {
