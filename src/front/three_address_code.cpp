...
TemporaryAddressPtr Address::temporary(ThreeAddressCode *src,
                                       const std::string &name) {
  return std::make_shared<TemporaryAddress>(src, name);
}

FunctionAddressPtr
Address::function(std::shared_ptr<std::vector<ThreeAddressCodePtr>> codes,
                  const std::string &name) {
...
std::string ThreeAddressCode::to_string() const {
  ...
  case TACOperator::MUL: {
    result_str << "  "
               << result_->to_string() + " = " + argL_->to_string() + " * " +
                      argR_->to_string() + ";";
    break;
  }
  ...
  case TACOperator::IF_TRUE_GOTO: {
    result_str << "  "
               << "if " + argL_->to_string() + " goto " +
                      jump_dest_->to_string() + ";";
    break;
  }
  case TACOperator::IF_FALSE_GOTO: {
    result_str << "  "
               << "if-false " + argL_->to_string() + " goto " +
                      jump_dest_->to_string() + ";";
    break;
  }
  case TACOperator::GOTO: {
    result_str << "  "
               << "goto " + jump_dest_->to_string() + ";";
    break;
  }
  ...
  case TACOperator::ALLOCA: {
    result_str << "  "
               << "alloca " + argL_->to_string() + " size " +
                      std::to_string(alloca_bytes_) + ";";
    break;
  }
  default:
