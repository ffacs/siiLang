...
TokenPtr Token::plus() { return token(TokenType::PLUS, "+"); }

TokenPtr Token::hyphen() { return token(TokenType::HYPHEN, "-"); }

TokenPtr Token::asterisk() { return token(TokenType::ASTERISK, "*"); }

TokenPtr Token::slash() { return token(TokenType::SLASH, "/"); }
...
TokenPtr Token::semicolon() { return token(TokenType::SEMICOLON, ";"); }

TokenPtr Token::equal() { return token(TokenType::EQUAL, "=="); }

TokenPtr Token::not_equal() { return token(TokenType::NOT_EQUAL, "!="); }

TokenPtr Token::left_angle() { return token(TokenType::LEFT_ANGLE, "<"); }

TokenPtr Token::less_equal() { return token(TokenType::LESS_EQUAL, "<="); }

TokenPtr Token::right_angle() { return token(TokenType::RIGHT_ANGLE, ">"); }

TokenPtr Token::greater_equal() {
  return token(TokenType::GREATER_EQUAL, ">=");
}

TokenPtr Token::assgin() { return token(TokenType::ASSIGN, "="); }

TokenPtr Token::left_brace() { return token(TokenType::LEFT_BRACE, "{"); }

TokenPtr Token::right_brace() { return token(TokenType::RIGHT_BRACE, "}"); }
...
TokenPtr Token::comma() { return token(TokenType::COMMA, ","); }

TokenPtr Token::left_bracket() { return token(TokenType::LEFT_BRACKET, "["); }

TokenPtr Token::right_bracket() { return token(TokenType::RIGHT_BRACKET, "]"); }
...
public:
  LexerImpl(std::istream &input)
      : index_(0), contents_(std::istreambuf_iterator<char>(input),
...
private:
  ...
  TokenPtr next_integer() {
    ...
    return Token::integer(
        std::string_view(contents_.c_str() + begin, index_ - begin));
  }
