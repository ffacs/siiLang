...
class TypeBuilder {
 public:
  virtual ~TypeBuilder() {}
  virtual void pointer_of(uint64_t offset_limit = std::numeric_limits<uint64_t>::max()) = 0;
...
struct Type {
  ...
  static TypePtr function(TypePtr return_type, std::vector<DeclaratorPtr> parameter_types);
...
struct PointerType : public Type {
  ...
  constexpr static uint64_t OFFSET_UNLIMIT = std::numeric_limits<uint64_t>::max();
  PointerType(TypePtr aim_type, uint64_t offset_limit) 
    : Type(TypeKind::POINTER),
      aim_type_(std::move(aim_type)), offset_limit_(offset_limit) {}
...
struct Declarator {
  ...
  Declarator(TypePtr type, const std::string& identifier) : type_(std::move(type)), identifier_(identifier) {}
  bool operator==(const Declarator& other) const {
    if (type_ == nullptr) { return other.type_ == nullptr; }
    if (type_ != nullptr && other.type_ == nullptr) { return false; } 
