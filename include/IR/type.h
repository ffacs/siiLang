...
struct IntegerType : public Type {
  size_t num_bits_;
  IntegerType(size_t num_bits) : Type(Type::Kind::INT), num_bits_(num_bits) {}
...
struct ArrayType : public Type {
  ...
  ArrayType(TypePtr element_type, int64_t element_count)
      : Type(Type::Kind::ARRAY), element_type_(std::move(element_type)),
...
struct FunctionType : public Type {
  ...
  FunctionType(TypePtr return_type, std::vector<TypePtr> parameter_types)
      : Type(Type::Kind::FUNCTION), return_type_(std::move(return_type)),
