...
namespace SiiIR {
  ...
  template < typename ValueType >
  class ListNode
      : public std::enable_shared_from_this< ListNode< ValueType > > {
  private:
  ...
  public:
    ...
    shared() const {
      return std::dynamic_pointer_cast< ValueType >(
    ...
    bool operator==(
        const ListIterator< ValueType, ParentType, true >& other ) const {
  ...
  public:
    ...
    void insert_after( const IterType&             iter,
    ...
    void insert_before( const IterType&             iter,
                        std::shared_ptr< NodeType > node ) {
      size_++;
      NodeType& iter_node    = *iter.node_;
